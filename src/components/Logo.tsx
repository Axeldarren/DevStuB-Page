...
const Logo = ({ className = "", size = "md", animate = false }: { className?: string; size?: "sm" | "md" | "lg"; animate?: boolean }) => {
  ...
  return (
    <div
      className={`flex flex-col sm:flex-row items-center gap-2 sm:gap-4 ${animate ? 'animate-logo-entrance' : ''} ${className}`}
    ...
    >
      ...
      <span
        className={`font-bold text-center sm:text-left ${size === 'lg' ? 'text-4xl' : size === 'md' ? 'text-2xl' : 'text-xl'} bg-gradient-to-r from-primary via-secondary to-accent bg-clip-text text-transparent`}
